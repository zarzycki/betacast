...
atmDataType=4             # 1 = GFS analysis, 2 = ERA-interim, 3 = CFSR ; 4 = preexisting initial conditions
...
casename=hindcast_mp15a-120a-US_CAM5_L30
...
############################### GET ATM DATA ############################### 
  ...
  if [ $atmDataType -eq 1 ] ; then
      ...
      echo "Using pre-existing initial conditions"
  else
...
############################### ATM NCL ############################### 
  ...
  then
     ...
     ncl -n atm_to_cam.ncl 'datasource="CFSR"'     \
       ...
       'wgt_filename="'${gfs2seWeights}'"' \
       'se_inic = "'${sePreFilterIC}'"'
  elif [ $atmDataType -eq 4 ] # CFSR
  then
     echo "Setting pre-existing initial conditions"
     sePreFilterIC=/glade/scratch/zarzycki/MPAS/init/GFS/mp15a-120a-US/${yearstr}${monthstr}${daystr}${cyclestr}/x8.195266.init.${yearstr}${monthstr}${daystr}${cyclestr}.nc
     sePostFilterIC=${sePreFilterIC}
  else
      echo "Incorrect model IC entered"
      exit 1
  fi
  ...
  # Need to add a check for case 4 when it doesn't actually use NCL!
  if [[ $? -ne 9 && $atmDataType -ne 4 ]]
  then
    echo "NCL exited with non-9 error code"
    exit 240
  fi
